        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
          <div className="space-y-4">
            <section>
              ...
              <div className="space-y-2 text-sm">
                ...
                {lead.website && <div className="flex items-center gap-2"><Globe className="w-4 h-4 text-muted-foreground shrink-0" /><a href={lead.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline truncate">{lead.website}</a></div>}
                ...
                {lead.google_maps_url && <a href={lead.google_maps_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-primary text-xs hover:underline"><ExternalLink className="w-3 h-3" /> Ver no Google Maps</a>}
              </div>
            </section>
            ...
            {(lead.responsavel || lead.descricao) && (
              <section>
                ...
                {lead.responsavel && <p className="text-sm"><span className="text-muted-foreground">Responsável:</span> {lead.responsavel}</p>}
                {lead.descricao && <p className="text-sm mt-1"><span className="text-muted-foreground">Descrição:</span> {lead.descricao}</p>}
              </section>
            )}
          </div>
          ...
          <div className="space-y-4">
            <section>
              ...
              <Input placeholder="https://chat.whatsapp.com/..." value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} />
              {whatsapp && <a href={whatsapp} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline mt-1 inline-block">Abrir grupo</a>}
            </section>

            <section>
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Agendar Reuniões</h3>
              <Calendar mode="single" selected={undefined} onSelect={handleDateSelect} modifiers={{ booked: selectedDates }} modifiersClassNames={{ booked: 'bg-primary text-primary-foreground rounded-md' }} className={cn("p-3 pointer-events-auto border rounded-md")} />
              {selectedDates.length > 0 && (
                <div className="mt-2 space-y-1">
                  ...
                  {selectedDates.sort((a, b) => a.getTime() - b.getTime()).map((d, i) => (
                    <div key={i} className="flex items-center justify-between text-xs bg-secondary rounded px-2 py-1">
                      <span className="font-mono-num">{d.toLocaleDateString('pt-BR')}</span>
                      <a href={generateGCalUrl(d)} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Google Calendar</a>
                    </div>
                  ))}
                </div>
              )}
