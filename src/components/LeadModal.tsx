...
import { MapPin, Phone, Globe, Star, MessageCircle, Instagram, ExternalLink } from 'lucide-react';
...
export function LeadModal({ lead, open, onClose, onUpdate }: Props) {
  ...
  const generateGCalUrl = (date: Date) => {
    ...
    return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=Reunião - ${encodeURIComponent(lead.name)}&dates=${start}/${end}&details=${encodeURIComponent(`Reunião com ${lead.name}`)}`;
  };
    ...
    <Dialog open={open} onOpenChange={handleOpen}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-sm text-muted-foreground">{lead.title} • {lead.category}</p>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
            ...
            <section>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-muted-foreground text-xs">{lead.city}, {lead.state}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Phone className="w-4 h-4 text-muted-foreground shrink-0" />
                  <span className="font-mono-num">{lead.phone || '—'}</span>
                </div>
                {lead.website && (
                  <div className="flex items-center gap-2">
                ...
                {lead.google_maps_url && (
                  <a href={lead.google_maps_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-primary text-xs hover:underline">
            ...
            <section>
              ...
              <div className="flex gap-4">
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-mono-num font-medium">{lead.rating || '—'}</span>
                </div>
                <div className="text-sm text-muted-foreground">
                  <span className="font-mono-num font-medium text-foreground">{lead.reviews_count || '0'}</span> avaliações
                </div>
              </div>
            </section>
          ...
          <div className="space-y-4">
            <section>
              ...
              />
              {whatsapp && (
                <a href={whatsapp} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline mt-1 inline-block">
                  Abrir grupo
                </a>
              )}
            </section>
            ...
            <section>
              ...
              <Calendar
                ...
                modifiers={{ booked: selectedDates }}
                ...
                <div className="mt-2 space-y-1">
                  ...
                  {selectedDates.sort((a, b) => a.getTime() - b.getTime()).map((d, i) => (
                    <div key={i} className="flex items-center justify-between text-xs bg-secondary rounded px-2 py-1">
                      ...
                      <a href={generateGCalUrl(d)} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
                        Google Calendar
                      </a>
                    </div>
                  ))}
