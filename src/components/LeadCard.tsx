...
const statusCardBorder: Record<string, string> = {
  analise_pendente: 'border-l-4 border-l-[hsl(var(--status-pending-border))] bg-[hsl(var(--status-pending)/0.3)]',
  em_analise: 'border-l-4 border-l-[hsl(var(--status-analyzing-border))] bg-[hsl(var(--status-analyzing)/0.3)]',
  follow_up: 'border-l-4 border-l-[hsl(var(--status-followup-border))] bg-[hsl(var(--status-followup)/0.3)]',
  reuniao_agendada: 'border-l-4 border-l-[hsl(var(--status-meeting-border))] bg-[hsl(var(--status-meeting)/0.3)]',
  recusado: 'border-l-4 border-l-[hsl(var(--status-refused-border))] bg-[hsl(var(--status-refused)/0.3)]',
  venda_fechada: 'border-l-4 border-l-[hsl(var(--status-closed-border))] bg-[hsl(var(--status-closed)/0.3)]',
};
      ...
      <div className={`rounded-xl bg-card p-4 shadow-sm ${borderClass} space-y-3`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={saveEdit} className="h-9 px-4"><Check className="w-4 h-4 mr-1" /> Salvar</Button>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {([
            ...
            <div key={key} className={key === 'descricao' || key === 'website' ? 'sm:col-span-2' : ''}>
              ...
              <Input className="h-10 text-sm mt-0.5" value={(editData[key] as string) || ''} onChange={(e) => setEditData(prev => ({ ...prev, [key]: e.target.value }))} />
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className={`rounded-xl bg-card p-4 shadow-sm hover:shadow-md transition-shadow ${borderClass}`}>
      ...
      <div className="mt-3 flex gap-2">
        <Button variant="outline" size="sm" className="flex-1 h-10 text-sm" onClick={startEdit}><Pencil className="w-4 h-4 mr-1.5" /> Editar</Button>
        <Button size="sm" className="flex-1 h-10 text-sm" onClick={() => onOpenLead(lead)}><Eye className="w-4 h-4 mr-1.5" /> Detalhes</Button>
        <Button variant="ghost" size="sm" className="h-10 w-10 p-0 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => onDeleteLead(lead.id)}><Trash2 className="w-4 h-4" /></Button>
      </div>
    </div>
  );
}
