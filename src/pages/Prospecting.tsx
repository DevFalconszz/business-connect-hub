...
import { Search, Plus, Loader2, Globe, ExternalLink, AlertTriangle, CheckCircle } from 'lucide-react';
      ...
      <main className="max-w-[1400px] mx-auto px-4 py-6">
        <Card className="mb-6 border-none shadow-md bg-card">
          <CardHeader className="pb-3">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Mostra apenas estabelecimentos <strong>sem site</strong> ou <strong>sem anúncios/campanhas ativas</strong> — oportunidades reais de venda.
            </p>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col sm:flex-row gap-3">
              <Input className="h-12 text-sm rounded-xl flex-1" placeholder="Nicho (ex: Clínica Odontológica, Restaurante...)" value={niche} onChange={(e) => setNiche(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} />
              <Input className="h-12 text-sm rounded-xl flex-1" placeholder="Cidade (ex: São Paulo, Curitiba...)" value={city} onChange={(e) => setCity(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} />
              <Button className="h-12 rounded-xl px-8 text-sm font-semibold min-w-[140px]" onClick={handleSearch} disabled={loading}>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-sm text-muted-foreground">Buscando e filtrando estabelecimentos sem presença digital...</p>
          </div>
        )}
                  ...
                  <TableBody>
                    {results.map((result, i) => (
                      <TableRow key={i} className="hover:bg-muted/50">
                        <TableCell>
                          <Button size="sm" className="h-8 w-8 p-0 rounded-lg" onClick={() => { setAddingIndex(i); setResponsavel(''); }} title="Adicionar aos leads">
                            <Plus className="w-4 h-4" />
                          </Button>
                        </TableCell>
                        <TableCell className="font-medium text-sm max-w-[200px] truncate">{result.name || '—'}</TableCell>
                        ...
                        <TableCell>
                          {result.has_website ? (
                            <Badge variant="outline" className="text-xs gap-1"><CheckCircle className="w-3 h-3 text-green-500" />Sim</Badge>
                          ) : (
                        ...
                        <TableCell>
                          {result.has_ads ? (
                            <Badge variant="outline" className="text-xs gap-1"><CheckCircle className="w-3 h-3 text-green-500" />Ativo</Badge>
                          ) : (
          ...
          {addingIndex !== null && results[addingIndex] && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input className="h-12 text-sm rounded-xl mt-1.5" placeholder="Nome do responsável por este lead" value={responsavel} onChange={(e) => setResponsavel(e.target.value)} autoFocus />
              </div>
              <Button className="w-full h-12 rounded-xl text-sm font-semibold" onClick={() => handleAddLead(results[addingIndex])}>
