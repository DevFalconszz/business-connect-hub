 ...
 * de uso reais (tabela serpapi_usage_snapshots, atualizada por cron/sync).
 * Assim a chave principal esgotada é PULADA de forma proativa e a fallback
 * passa a ser usada em seu lugar, sem gerar erros no sistema.
 */
async function refreshKeyHealth() {
  const now = Date.now();
  if (now - lastHealthCheckMs < 30_000) return; // cache curto para páginas seguidas
...
function getActiveKey(): string {
  ...
  // Todas esgotadas: tenta mesmo assim (o laço vai esgotar o retry com erro claro)
  currentKeyIndex = 0;
...
async function serpSearchWithFallback(niche: string, city: string, start: number, num: number) {
  ...
  for (let attempt = 0; attempt < SERPAPI_KEYS.length; attempt++) {
    ...
    if (res.ok) {
      ...
      // Chave esgotada/inválida: a SerpApi retorna 200 com `error` no corpo.
      // Detecta e roda para a próxima chave em vez de propagar o erro ao usuário.
      if (!data || data.error) {
        const detail = data?.error ? String(data.error) : 'JSON inválido';
        await logApiUsage('search-leads-paged/search', idx, 'error', res.status, detail);
        rotateKey();
        continue;
      }
...
async function serpPlaceWithFallback(placeId: string, dataId: string, lat: string, lng: string) {
  ...
  for (let attempt = 0; attempt < SERPAPI_KEYS.length; attempt++) {
    ...
    if (res.ok) {
      ...
      // Chave esgotada: troca para a próxima em vez de retornar erro/resultado vazio
      if (!data || data.error) {
        const detail = data?.error ? String(data.error) : 'JSON inválido';
        await logApiUsage('search-leads-paged/place', idx, 'error', res.status, detail);
        rotateKey();
        continue;
      }
